...
class Packager {
  ...
  callArtifactBuildStarted(event, logFields) {
    ...
    return (0, _bluebirdLst().coroutine)(function* () {
      ...
      const handler = (0, _platformPackager().resolveFunction)(_this2.config.artifactBuildStarted, "artifactBuildStarted");

      if (handler != null) {
        yield Promise.resolve(handler(event));
      }
    })();
  }
  /**
  ...
  callArtifactBuildCompleted(event) {
    ...
    return (0, _bluebirdLst().coroutine)(function* () {
      _this3.dispatchArtifactCreated(event);

      const handler = (0, _platformPackager().resolveFunction)(_this3.config.artifactBuildCompleted, "artifactBuildCompleted");
  ...
  build() {
    ...
    return (0, _bluebirdLst().coroutine)(function* () {
      let configPath = null;
      let configFromOptions = _this4.options.config;
      ...
      const projectDir = _this4.projectDir;
      const devPackageFile = path.join(projectDir, "package.json");
      _this4._devMetadata = yield (0, _promise().orNullIfFileNotExist)((0, _packageMetadata().readPackageJson)(devPackageFile));
      const devMetadata = _this4.devMetadata;
      ...
      _this4._appDir = yield (0, _config().computeDefaultAppDirectory)(projectDir, configuration.directories.app);
      _this4.isTwoPackageJsonProjectLayoutUsed = _this4._appDir !== projectDir;
      const appPackageFile = _this4.isTwoPackageJsonProjectLayoutUsed ? path.join(_this4.appDir, "package.json") : devPackageFile; // tslint:disable:prefer-conditional-expression

      if (_this4.devMetadata != null && !_this4.isTwoPackageJsonProjectLayoutUsed) {
        _this4._metadata = _this4.devMetadata;
      } else {
        _this4._metadata = yield _this4.readProjectMetadataIfTwoPackageStructureOrPrepacked(appPackageFile);
      }

      (0, _builderUtil().deepAssign)(_this4.metadata, configuration.extraMetadata);

      if (_this4.isTwoPackageJsonProjectLayoutUsed) {
        _builderUtil().log.debug({
          devPackageFile,
          appPackageFile
      ...
      (0, _packageMetadata().checkMetadata)(_this4.metadata, _this4.devMetadata, appPackageFile, devPackageFile);
      return yield _this4._build(configuration, _this4._metadata, _this4._devMetadata);
    })();
  ...
  _build(configuration, metadata, devMetadata, repositoryInfo) {
    ...
    return (0, _bluebirdLst().coroutine)(function* () {
      yield (0, _config().validateConfig)(configuration, _this5.debugLogger);
      _this5._configuration = configuration;
      _this5._metadata = metadata;
      _this5._devMetadata = devMetadata;

      if (repositoryInfo != null) {
        _this5._repositoryInfo.value = Promise.resolve(repositoryInfo);
      }

      _this5._appInfo = new (_appInfo().AppInfo)(_this5, null);
      _this5._framework = yield createFrameworkInfo(_this5.config, _this5);
      const commonOutDirWithoutPossibleOsMacro = path.resolve(_this5.projectDir, (0, _macroExpander().expandMacro)(configuration.directories.output, null, _this5._appInfo, {
        os: ""
      }));
      ...
      _this5.artifactCreated(event => {
        if (event.file != null) {
          artifactPaths.add(event.file);
        }
      });

      const platformToTargets = yield (0, _promise().executeFinally)(_this5.doBuild(),
      /*#__PURE__*/
      (0, _bluebirdLst().coroutine)(function* () {
        if (_this5.debugLogger.isEnabled) {
          yield _this5.debugLogger.save(path.join(commonOutDirWithoutPossibleOsMacro, "builder-debug.yml"));
        }

        yield _this5.tempDirManager.cleanup();
      }));
      return {
  ...
  readProjectMetadataIfTwoPackageStructureOrPrepacked(appPackageFile) {
    ...
    return (0, _bluebirdLst().coroutine)(function* () {
      ...
      data = yield (0, _promise().orNullIfFileNotExist)((0, _asar().readAsarJson)(path.join(_this6.projectDir, "app.asar"), "package.json"));

      if (data != null) {
        _this6._isPrepackedAppAsar = true;
        return data;
      }
  ...
  doBuild() {
    ...
    return (0, _bluebirdLst().coroutine)(function* () {
      const taskManager = new (_builderUtil().AsyncTaskManager)(_this7.cancellationToken);
      const platformToTarget = new Map();
      const createdOutDirs = new Set();

      for (const [platform, archToType] of _this7.options.targets) {
        if (_this7.cancellationToken.cancelled) {
          break;
        }
        ...
        const packager = _this7.createHelper(platform);
        ...
        for (const [arch, targetNames] of (0, _targetFactory().computeArchToTargetNamesMap)(archToType, packager.platformSpecificBuildOptions, platform)) {
          ...
          yield _this7.installAppDependencies(platform, arch);
          ...
          const outDir = path.resolve(_this7.projectDir, packager.expandMacro(_this7._configuration.directories.output, _builderUtil().Arch[arch]));
  ...
  installAppDependencies(platform, arch) {
    ...
    return (0, _bluebirdLst().coroutine)(function* () {
      if (_this8.options.prepackaged != null || _this8.framework.isNpmRebuildRequired !== true) {
        return;
      }

      const frameworkInfo = {
        version: _this8.framework.version,
        useCustomDist: _this8.config.muonVersion == null
      };
      const config = _this8.config;
      ...
      if (beforeBuild != null) {
        const performDependenciesInstallOrRebuild = yield beforeBuild({
          appDir: _this8.appDir,
          electronVersion: _this8.config.electronVersion,
          platform,
          arch: _builderUtil().Arch[arch]
        }); // If beforeBuild resolves to false, it means that handling node_modules is done outside of electron-builder.

        _this8._nodeModulesHandledExternally = !performDependenciesInstallOrRebuild;
      ...
      if (config.buildDependenciesFromSource === true && platform.nodeName !== process.platform) {
        ...
        yield (0, _yarn().installOrRebuild)(config, _this8.appDir, {
          frameworkInfo,
          platform: platform.nodeName,
          arch: _builderUtil().Arch[arch],
          productionDeps: _this8.productionDeps
        });
      }
    })();
  }
